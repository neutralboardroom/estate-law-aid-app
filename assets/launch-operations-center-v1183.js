(() => {
  ...
  const localFallback = () => ({schema:'ela-unified-live-operations-v1',version:'1.1.83',product:'Estate Law Aid',decision:'CONDITIONAL_GO',dependencies:[{label:'Operations evidence',state:'CONTRACT_READY_NOT_CONNECTED',gate:'BLOCKED',evidence:'Backend status is unavailable. Use the packaged data/unified_live_operations_v1183.json evidence file.',ownerAction:'NONE'}]});
  const load = async () => {
    summary.textContent = 'Checking local portal evidence…';
    try {
      const response = await fetch('/api/v1183/unified-live-operations',{headers:{accept:'application/json'}});
  ...
  const exportSnapshot = () => {
    ...
    a.href=url; a.download='estate-law-aid-launch-operations-v1.1.83.json'; a.click(); URL.revokeObjectURL(url);
  };
