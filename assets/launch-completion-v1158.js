(function(root,factory){
  ...
  root.ELA_V1158_LAUNCH_COMPLETION=api;
  ...
  'use strict';
  const KEY='ela:v1158:launch-completion';
  ...
  const CHECK_IDS=Object.freeze(['manual_keyboard_zoom','manual_screen_reader','physical_device','owner_go_no_go']);
  ...
  function assess(model,state){
    ...
    const capabilities=(Array.isArray(model&&model.launchCapabilityMatrix)?model.launchCapabilityMatrix:[]).map(row=>({
      capabilityId:row.capabilityId,tier:row.tier,capability:row.capability,packagedState:row.state,environment:row.environment,live:Boolean(row.live),ownerActionId:row.ownerActionId||null,
      deviceEvidenceRecorded:row.capabilityId==='A-MANUAL-ACCESSIBILITY'?Boolean(checks.manual_keyboard_zoom||checks.manual_screen_reader||checks.physical_device):row.capabilityId==='A-PRODUCTION'?Boolean(checks.owner_go_no_go):false,
      truth:row.truth||''
    }));
    const ownerActions=(Array.isArray(model&&model.ownerActions)?model.ownerActions:[]).map(row=>({requestId:row.requestId,packagedState:row.state,deviceState:actions[row.requestId]||row.state,canonicalReceiptState:row.canonicalReceiptState,legacyAliases:row.legacyAliases||[]}));
    return {
      version:model.version,
      selectedScope:model.selectedScope,
      capabilities,
      integrationBoundaries:(model.integrationMaturity&&model.integrationMaturity.boundaries)||[],
      ownerActions,
      checks,
      deployment:false,D4:false,D5:false,liveClaims:false,billing:false,opportunities:false,uploadsOCR:false,externalAI:false,filing:false,
      canonicalReceipt:'PENDING_CANONICAL_RECEIPT',
      truth:'Device-only entries are preparation notes. They do not change the packaged launch matrix, verify an owner action, mutate the canonical Smarter Justice queue, deploy the portal, or open a live gate.'
    };
  }
  function exportPacket(model,state){return {
    schema:'estate-law-aid-launch-completion-export',version:'1.1.58',exportedAt:new Date().toISOString(),exportMode:'user-directed-device-download',
    modelVersion:model.version,releaseCandidateId:model.releaseBinding.releaseCandidateId,deviceState:clean(state),assessment:assess(model,state),
    canonicalCoordination:model.canonicalCoordination,boundaries:model.browserLocal
  };}
