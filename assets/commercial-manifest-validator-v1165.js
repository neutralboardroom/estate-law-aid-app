(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.ELA_V1165_COMMERCIAL_MANIFEST=api;if(root&&root.document)root.addEventListener('DOMContentLoaded',()=>api.bind(root.document));})(typeof window!=='undefined'?window:globalThis,function(){
'use strict';
...
function bind(doc){const form=doc.getElementById('commercialManifestValidatorV1165');if(!form)return;const area=doc.getElementById('commercialManifestJsonV1165'),status=doc.getElementById('commercialManifestStatusV1165'),out=doc.getElementById('commercialManifestResultV1165'),file=doc.getElementById('commercialManifestFileV1165');let last=null;const render=r=>{last=r;status.textContent=r.compatible?'Compatible for scoped canonical-receipt review. No billing or live gate changed.':`Not compatible: ${r.errors.length} issue(s). No billing or live gate changed.`;out.textContent=JSON.stringify(r,null,2)};file.addEventListener('change',async()=>{const f=file.files&&file.files[0];if(f)area.value=await f.text()});form.addEventListener('click',async e=>{const a=e.target&&e.target.dataset&&e.target.dataset.manifestAction;if(!a)return;if(a==='sample'){area.value=JSON.stringify(sample(),null,2);status.textContent='Safe non-live sample loaded.';return}if(a==='clear'){area.value='';out.textContent='';last=null;file.value='';status.textContent='Cleared. No data was saved or sent.';return}if(a==='validate'){try{const raw=area.value;const manifest=JSON.parse(raw);const r=validate(manifest);r.inputSha256=await sha256Text(raw);r.validatedAt=new Date().toISOString();render(r)}catch(err){render({compatible:false,errors:[`Invalid JSON: ${err.message}`],warnings:[],checks:[],canonicalReceiptRecorded:false,portalBillingAuthority:false,billingActivationAllowed:false,liveAccepted:false})}return}if(a==='export'&&last)download('estate-law-aid-commercial-manifest-compatibility-v1.1.65.json',JSON.stringify(last,null,2),'application/json')})}
